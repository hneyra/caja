package kamayuk.caja.plataforma.identidad;
...
import kamayuk.caja.compartido.CiudadanoContext;
import kamayuk.caja.compartido.TenantContext;
import kamayuk.caja.dominio.DocumentoIdentidad;
 ...
 * </ul>
 *
 * <h2>Por que sigue aqui, si `caja` no publica ni una ruta de ciudadano</h2>
 *
 * <p>Porque lo que mide <b>no</b> es una pantalla del portal: es {@link SeguridadWeb}, que en el
 * {@code src/main} de este sistema registra <b>las dos</b> cadenas, y los dos filtros de sujeto de
 * {@code plataforma.tenant}, que se reparten el camino preguntando {@link
 * SeguridadWeb#esDelPortal}. Ese reparto esta vivo aqui: {@code TenantContextFilter} <b>no fija
 * contexto de tenant</b> bajo {@code /caja/api/v1/portal}, y la cadena del portal se lleva ese
 * prefijo antes que la general. Borrar esta prueba dejaria ese codigo —codigo de esta
 * municipalidad, no heredado en un comentario— sin nadie que lo mida.
 *
 * <p>Y la consecuencia de que hoy no haya controlador debajo es que el prefijo esta <b>libre</b>:
 * el dia que alguien mapee ahi un endpoint de funcionario, la peticion no la valida el emisor de
 * funcionarios y no llega con municipalidad. Que eso se note es exactamente lo que estas pruebas
 * hacen ruidoso.
 *
...
class CadenaDelPortalTest {
            ...
            assertThat(DelPortal.SUJETO.get())
                    ...
                    .isEqualTo(
                            new DocumentoIdentidad(
                                    kamayuk.caja.dominio.TipoDocumento.DNI, "03593174"));
        }
    ...
    static class ContextoDePrueba {
        ...
        org.springframework.boot.web.servlet.FilterRegistrationBean<
                        kamayuk.caja.plataforma.tenant.TenantContextFilter>
                filtroDeTenant() {
            var registro =
                    new org.springframework.boot.web.servlet.FilterRegistrationBean<>(
                            new kamayuk.caja.plataforma.tenant.TenantContextFilter());
            registro.setOrder(0);
            return registro;
        }
        ...
        @Bean
        org.springframework.boot.web.servlet.FilterRegistrationBean<
                        kamayuk.caja.plataforma.tenant.DocumentoCiudadanoContextFilter>
                filtroDelCiudadano() {
            var registro =
                    new org.springframework.boot.web.servlet.FilterRegistrationBean<>(
                            new kamayuk.caja.plataforma.tenant.DocumentoCiudadanoContextFilter());
            registro.setOrder(0);
            return registro;
        }
    ...
    static class DelPortal {
        ...
        static final String RUTA = SeguridadWeb.RAIZ_DEL_PORTAL + "/prueba";
    ...
    static class DeFuncionario {
        ...
        static final String RUTA = Api.RAIZ + "/prueba/municipalidad";
        static final AtomicInteger LLAMADAS = new AtomicInteger();
