...
class CadenaDeIdentidadTest {
    ...
    class Accesos {
        ...
        @Test
        @DisplayName("con el claim de A, se ven las cajas de A")
        void conElClaimDeAseVenLosDeA() throws Exception {
            ...
            assertThat(respuesta.body()).contains("C-A");
            assertThat(ControladorDePrueba.LLAMADAS.get()).isEqualTo(1);
        }

        @Test
        @DisplayName("con el claim de A no se ve ni una caja de B, y al reves tampoco")
        void unaMunicipalidadNoVeLaOtra() throws Exception {
            ...
            assertThat(deA).contains("C-A").doesNotContain("C-B");
            assertThat(deB).contains("C-B").doesNotContain("C-A");
        }
    ...
    /**
     * Lee {@code caja}, que es una tabla de tenant con RLS.
     *
     * <p>La tabla no es un detalle: esta prueba mide que el claim acabe <b>filtrando filas</b>, asi
     * que tiene que leer una que lleve politica de RLS y que {@link DatosDePrueba#sembrarTenant}
     * siembre en las dos municipalidades con un valor distinto en cada una. {@code caja} cumple las
     * dos —{@code caja_tenant}, y el codigo {@code C-A} frente a {@code C-B}—, y ademas es la
     * ventanilla, que es de lo que trata este sistema.
     *
     * <p><b>La ruta va bajo {@link Api#RAIZ}</b> y no bajo una escrita a mano. Es lo unico que hace
     * que la peticion caiga en {@code RAIZ_DE_LA_API} de {@link SeguridadWeb}: fuera de ahi la
     * recoge el {@code anyRequest().denyAll()} final y todo contesta 403 {@code SIN_PRIVILEGIO} —
     * un rojo que se parece a un problema de permisos y es un problema de prefijo.
     *
    ...
    static class ControladorDePrueba {
        ...
        static final String RUTA = Api.RAIZ + "/prueba/cajas";
        ...
        public List<String> codigos() {
            ...
            return plantilla.queryForList("SELECT codigo FROM caja ORDER BY codigo", String.class);
        }
    }
}
