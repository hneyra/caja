package kamayuk.caja.plataforma.tenant;
...
import kamayuk.caja.esquema.DatosDePrueba;
...
class AislamientoConElPoolTest {
    ...
    void elContextoNoSobreviveALaDevolucionDeLaConexion() {
        try (Entorno entorno = new Entorno(1)) {
            long vistos = entorno.enContextoDe(municipalidadA, this::cajas);
            assertThat(vistos).as("con contexto, la municipalidad A ve su caja").isEqualTo(1);

            // Misma conexion fisica: el pool tiene una sola.
            Throwable fallo = entorno.sinContexto(this::cajas);
    ...
    void dosMunicipalidadesSobreLaMismaConexionNoSeVenEntreSi() {
        try (Entorno entorno = new Entorno(1)) {
            assertThat(entorno.enContextoDe(municipalidadA, this::cajasDeA)).isEqualTo(1);
            assertThat(entorno.enContextoDe(municipalidadB, this::cajasDeA))
                    .as("la municipalidad B no ve la caja de A ni reutilizando su conexion")
                    .isZero();
                ...
                tareas.add(
                        () -> {
                            ...
                            return entorno.enContextoDe(
                                            propia,
                                            jdbc -> {
                                                long propias = cajas(jdbc);
                                                long ajenas = contarPor(jdbc, ajena);
                                                return propias == 1 && ajenas == 0 ? 1L : 0L;
                                            })
                                    == 1L;
                        });
            }
    ...
    void sinElGuardiaLaConexionContaminadaSeReutiliza() {
        ...
        try (Entorno entorno = new Entorno(1, false)) {
            ...
            long vistosSinContexto = entorno.enContextoDe(null, this::cajas);
    ...
    private long medir(Entorno entorno, int repeticiones) {
        ...
        for (int i = 0; i < repeticiones; i++) {
            entorno.enContextoDe(municipalidadA, this::cajas);
        }
        return System.nanoTime() - inicio;
    }

    /**
     * Cuenta las ventanillas que la conexion deja ver.
     *
     * <p>Es {@code caja} y no cualquier otra tabla porque tiene las tres cosas que esta prueba
     * necesita a la vez: lleva {@code municipalidad_id NOT NULL}, tiene su politica de RLS ({@code
     * caja_tenant}), y {@link DatosDePrueba#sembrarTenant} escribe <b>exactamente una</b> en cada
     * municipalidad. Esa ultima es la que convierte «2 donde debe haber 1» en una fuga y no en un
     * dato de mas.
     *
     * <p>Una tabla sin RLS —o sembrada en una sola municipalidad— dejaria estas seis pruebas
     * pasando sin haber medido nada del aislamiento.
     */
    private long cajas(JdbcTemplate jdbc) {
        Long total = jdbc.queryForObject("SELECT count(*) FROM caja", Long.class);
        return total == null ? -1 : total;
    }

    private long cajasDeA(JdbcTemplate jdbc) {
    ...
    private long contarPor(JdbcTemplate jdbc, long municipalidadId) {
        Long total =
                jdbc.queryForObject(
                        "SELECT count(*) FROM caja WHERE municipalidad_id = ?",
                        Long.class,
                        municipalidadId);
        return total == null ? -1 : total;
    }
