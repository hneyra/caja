-- ============================================================================
--  V3 — LO QUE DEJA EL CONSUMIDOR DEL BUZON DE `identidad` (etapa 4 de
...
--  siguen en este baseline y el guardia las sigue leyendo sin un viaje de red
--  (D-N5); lo que cambia en esta etapa es DE DONDE SALE esa copia: la escribe un
--  consumidor que trae los hechos del buzon de `identidad`, los aplica uno a uno
--  y los acusa DESPUES de confirmar. Estas dos tablas son lo que ese consumidor
--  necesita recordar, y nada mas.
--
...
--     a escribir la copia. Es la misma tabla que `rentas` tiene para `catastro`
--     (`catastro_evento_aplicado`), por el mismo motivo.
--
--  2. `identidad_evento_muerto`: el evento que NO SE PODRA APLICAR NUNCA —cuerpo
--     ilegible, tipo desconocido—, apartado con su motivo. Se aparta, SE ACUSA y
--     se avisa a una persona: reintentarlo no lo arregla y bloquearia la cola
--     detras de el, con la copia local congelada sin un solo error visible. Lo
--     que NO se aparta aqui es lo que no se puede aplicar AHORA —la base caida,
--     el grupo que todavia no llego—: eso no se acusa y se reintenta.
--
--  `cuerpo` ES `text` Y NO `jsonb`, A PROPOSITO. Es la leccion de `V12` de
--  `rentas`: `jsonb` reordena las claves y descarta los espacios, asi que lo que
--  se lee no es byte a byte lo que se escribio, y un cuerpo que se aparto porque
--  NO SE PUDO LEER como JSON no cabe en una columna que exige que lo sea — el
--  motivo por el que se aparta es exactamente lo que impediria guardarlo.
--
...
COMMENT ON COLUMN identidad_evento_muerto.cuerpo IS
    '`text` y no `jsonb` (la leccion de `V12` de `rentas`): un cuerpo que se aparto por no ser '
    'JSON no cabe en una columna que exija que lo sea, y `jsonb` ademas reordena las claves, asi '
    'que lo guardado dejaria de ser lo que se recibio.';
