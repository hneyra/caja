...
--  cuatro -postgis, pg_trgm, unaccent, btree_gist- y caja no usa ninguna: no
--  tiene geometria, ni busqueda por aproximacion, ni una sola columna GENERADA.
--  La que la obligaba era `nombre_normalizado`, que se retiro (ver mas abajo).
...
-- ----------------------------------------------------------------------------
--  LO QUE ESTE ARCHIVO NO TRAE, Y POR QUE (P5D)
-- ----------------------------------------------------------------------------
--
--  El generador de ADR-0032 restringe el esquema de `sgtm` a las tablas de este
--  sistema, pero arrastra TODA funcion y TODO dominio del esquema original. Es el
--  mismo defecto que P5B encontro en `normativa` (cuatro funciones) y P5C en
--  `catastro` (cinco). Aqui son OCHO funciones y CINCO dominios, y ninguno de los
--  trece lo delata el comparador de `baselines/verificar/`: ese mide fidelidad al
--  monolito -que si las tiene- y no PERTENENCIA a este sistema.
--
--  FUNCIONES RETIRADAS (8 de 9). La que queda es la unica con disparador aqui.
--
--    conjunto_sellado_es_inmutable()             normativa. Disparador de
--                                                `conjunto_parametros`, que aqui no esta.
--    detalle_de_conjunto_sellado_es_inmutable()  normativa. Su cuerpo consulta
--                                                `conjunto_parametros`.
--    valuacion_de_conjunto_sellado_es_inmutable() normativa/catastro. Idem.
--    valuacion_de_publicacion_sellada_es_inmutable() normativa. Consulta
--                                                `parametro_tributario`.
--    declaracion_jurada_estado_es_terminal()     rentas. Sin disparador aqui.
...
--    nombre_normalizado(text)                    catastro (`via.nombre_busqueda`) y
--                                                rentas (`contribuyente_nombre_trgm_ix`).
--                                                CAJA NO TIENE NI UNA COLUMNA GENERADA,
--                                                asi que nadie la llamaba — y arrastraba
--                                                la extension `unaccent`, que sin ella
--                                                habria hecho morir esta migracion en su
--                                                `CREATE FUNCTION`.
--
--  DOMINIOS RETIRADOS (5 de 7): `alicuota`, `area_m2`, `cod_catastral`, `monto_calc`
--  y `porcentaje`. NINGUNA de las 23 tablas los usa; son los tipos del calculo
--  tributario (rentas) y del predio (catastro). Quedan `dinero` -18 columnas- y
--  `ejercicio` -4, una de ellas la clave de particion de `auditoria`-.
--  Es una diferencia con P5B y P5C, que no retiraron dominios: alli no sobraba
--  ninguno. Un dominio muerto es inerte, y aun asi se va: declarar `alicuota` en el
--  esquema de la caja afirma que aqui se guarda un porcentaje tributario, y la
--  primera regla de este sistema es que no sabe que es un tributo (ADR-0026 §1).
--
--  UN DEFECTO DE SINTAXIS DEL GENERADOR, corregido: `recibo_turno_fk` se emitia con
--  `NOT VALID NOT VALID`. PostgreSQL lo acepta -acumula el atributo con OR- pero el
--  archivo deja de ser estable en ida y vuelta, y con checksum de Flyway eso importa.
--  El mismo defecto esta 37 veces en el baseline de `rentas`.
--
-- ----------------------------------------------------------------------------
--  EL `REVOKE` QUE EL MONOLITO NO PUDO HACER SOBRE `cierre_caja` (V32 §1.bis)
-- ----------------------------------------------------------------------------
--
--  Se copia aqui entero porque en este repositorio NO HAY `git log` donde
--  encontrarlo, que es el mismo motivo por el que los cinco hallazgos de RLS estan
--  en la cabecera. `V32` del monolito lo descubrio EJECUTANDOLO:
--
...
--
--  En PostgreSQL, `SELECT ... FOR UPDATE` exige el privilegio de UPDATE sobre la
--  tabla -tambien `FOR NO KEY UPDATE` y `FOR SHARE`-. Y esa fila era EL PUNTO DE
--  SERIALIZACION DE LA VENTANILLA: revocar el UPDATE no habria hecho el turno
--  inmutable, habria dejado la caja sin poder cobrar, con un sintoma -«bad SQL
--  grammar», porque el SQLSTATE 42501 cae en la clase 42- que no se parece a su causa.
--
--  ESTE ARCHIVO LO CONSERVA TAL CUAL: `cierre_caja` mantiene su UPDATE. `V2` de este
--  repositorio es la que lo replantea, y dice como: mueve la serializacion de la
--  ventanilla al turno... a la ORDEN DE COBRO, y entonces el REVOKE si se puede.
--  No se hace aqui para que el diff de `V2` enseñe las dos cosas juntas -por que se
--  podia y que se movio para poder-.
-- ----------------------------------------------------------------------------
-- ============================================================================
...
-- ==========================================================================
--  2. FUNCIONES
--  UNA, y es la del unico disparador de este esquema. Van antes que las
--  tablas por costumbre del generador; aqui ninguna columna las necesita,
--  porque CAJA NO TIENE NI UNA COLUMNA GENERADA. Un disparador sin su
--  funcion no protege nada, y una funcion sin su disparador no protege
--  nada tampoco: por eso aqui solo esta la que se dispara.
-- ==========================================================================
...
--
--  AQUI NO HAY NI UN PRIVILEGIO POR COLUMNA, y no es un olvido: el unico del
--  proyecto es el `GRANT UPDATE (estado) ON declaracion_jurada` de `V54`, que
--  es de `rentas`. Lo que si hay -y es lo mismo dicho al reves- es la AUSENCIA
--  de UPDATE donde el monolito lo revoco: `recibo` y `recibo_detalle` (V29),
--  `recibo_movimiento` (V30), `cierre_turno` y `cierre_turno_detalle` (V32) y
--  `auditoria` (V7) reciben solo INSERT y SELECT. Un volcado descuidado los
--  devuelve enteros, y entonces una tabla inmutable deja de serlo sin que se
--  vea en ningun diff.
-- ==========================================================================
