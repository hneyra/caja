 ...
 * peticion (#112; portado de {@code rentas}#316, {@code rentas@707aeb7}).
 *
 * <h2>Que se mide</h2>
 *
 * <p>{@code backend/*&#47;src/main} tiene 21 {@code LocalDate.now(reloj)} —el dia del turno, el de
 * la anulacion, la vigencia de un permiso, la fecha de la auditoria—, y los 21 leen el {@link
 * Clock} de {@link KamayukAplicacion#reloj()}. {@code LocalDate.now(Clock)} trunca con la zona
 * <b>del reloj</b>, asi que el dia de esos 21 sitios es el que diga la zona de ese bean. Esta
 ...
 *
 * <p>Es la franja en que el dia de Lima y el de UTC <b>discrepan</b>: a mediodia los dos dicen lo
 * mismo, y una muestra a mediodia sale verde con el defecto dentro —la muestra uniforme de #273—.
 * Por eso cada caso comprueba primero que en UTC su instante es <b>el dia siguiente</b>: si alguien
 * moviera la muestra fuera de la franja, esa premisa se caeria antes que la afirmacion.
 *
 ...
 */
@DisplayName("#112 — El dia del reloj es el de la municipalidad, no el del servidor")
class ElDiaDelRelojEsElDelProductoTest {
    ...
    @ParameterizedTest(name = "a las {1} de Lima ({0}), con la JVM en {2}, es {3}")
    @CsvSource({
    ...
    void elDiaEsElDeLaMunicipalidad(
        ...
        assertThat(LocalDate.now(aEsaHora))
                .as(
                        "LocalDate.now(reloj) —21 sitios de src/main— a las %s de la"
