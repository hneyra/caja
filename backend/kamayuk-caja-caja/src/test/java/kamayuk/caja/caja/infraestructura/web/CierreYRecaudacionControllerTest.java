...
import kamayuk.caja.caja.aplicacion.ArqueoDeTurno;
import kamayuk.caja.caja.aplicacion.CerrarTurno;
import kamayuk.caja.caja.aplicacion.ConsultaDeRecaudacion;
...
class CierreYRecaudacionControllerTest {
     ...
     *
     * <p>Que el cierre se pare con un pago sin entregar es una decision del caso de uso y se prueba
     * en {@code CerrarYArquearTest}; aqui se prueba el transporte, y meterle un evento pendiente
     * dejaria estas pruebas fallando por un motivo que no es el suyo.
     */
    private final ArqueoDeTurno arqueos = new ArqueoDeTurno(cierres, new BuzonEnMemoria());
    ...
    void laDistribucionPublicaElHueco() throws Exception {
        MvcResult resultado =
                mvc.perform(
                                MockMvcRequestBuilders.get("/caja/api/v1/recaudacion/por-area")
    ...
    private MvcResult cierre(String cuerpo) throws Exception {
        return mvc.perform(
                        MockMvcRequestBuilders.post("/caja/api/v1/turnos/cierre")
