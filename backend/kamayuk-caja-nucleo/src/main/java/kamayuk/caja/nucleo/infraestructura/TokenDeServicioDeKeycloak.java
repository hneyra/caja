...
/**
 * El token con el que la caja llama a otro sistema, pedido a Keycloak (#21 AC-2, ADR-0028 §2).
 *
 * <h2>Por que `client_credentials` y no el token de nadie</h2>
 *
 * <p>El publicador corre <b>sin usuario delante</b>: lo despierta un reloj, no una peticion, asi
 * que no hay ningun {@code Authorization} del que tirar. Lo que hay es un cliente confidencial suyo
 * —{@code kamayuk-caja-servicio-<ubigeo>}, uno por municipalidad— cuya cuenta de servicio lleva el
 ...
 * proceso con permiso sobre todas». Con un cliente por sistema, el token valdria para cualquiera.
 *
 * <h2>Se guarda, y se renueva antes de que caduque</h2>
 *
 * <p>Un token por pago serian dos viajes por evento y una carga de trabajo sobre Keycloak que crece
 * con la recaudacion. Se conserva hasta {@link #MARGEN} antes de su vencimiento; el margen existe
 * porque entre pedirlo y usarlo pasa tiempo, y un token que caduca en vuelo se ve exactamente igual
 * que una credencial mala: un 401.
 *
 * <h2>Lo que hace cuando no puede</h2>
 *
 * <p>Lanza {@link BuzonDelSistemaDeOrigen.NoContesta}, que se reintenta. No poder pedir el token
 * —el emisor caido, la clave equivocada, el cliente sin crear— <b>se arregla del lado del
 * despliegue y cambia solo</b>. Tratarlo como rechazo mataria el pago al primer intento, que es el
 * defecto que AC-3 cerro por su otro lado.
 */
@Component
public class TokenDeServicioDeKeycloak implements CredencialDeServicio {
    ...
    public TokenDeServicioDeKeycloak(
            JsonMapper json,
            Clock reloj,
            @Value("${kamayuk.caja.identidad.token:}") String punto,
            @Value("${kamayuk.caja.identidad.cliente:}") String clienteDeServicio,
            @Value("${kamayuk.caja.credencial:}") String clave) {
        this.json = json;
        this.reloj = reloj;
        this.punto = punto;
            ...
            if (token.isBlank()) {
                throw new BuzonDelSistemaDeOrigen.NoContesta(
                        "El emisor contesto 200 al pedir el token de «"
                                ...
                                + " esta emitiendo, no un pago rechazado");
            }
    ...
    private JsonNode pedir() {
        ...
        if (respuesta.statusCode() != 200) {
            ...
            // trae `error_description`, que puede repetir lo que se le mando. El mensaje acaba en
            // `pago_evento.ultimo_error`, que se guarda y se lee.
            throw new BuzonDelSistemaDeOrigen.NoContesta(
                    "El emisor contesto "
                            ...
                            + " (`reconciliar-identidades.sh servicios`, #21). No es el pago: es"
                            + " quien llama, y se arregla en el despliegue");
        }
        try {
