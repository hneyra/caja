...
| Pieza | Estado |
|---|---|
| `backend/kamayuk-caja-esquema` | **`V1__baseline.sql`** (23 tablas), **`V2__ordenes_de_cobro_y_outbox.sql`** y, desde la etapa 4 de ADR-0039, **`V3__consumidor_de_identidad.sql`**: `identidad_evento_aplicado` e `identidad_evento_muerto`, las dos con RLS `ENABLE`+`FORCE`, `INSERT, SELECT` para `kamayuk_app` y **sin `UPDATE` ni `DELETE`** —un acuse local no se edita—. El baseline se corrigió al traerlo: le sobraban **ocho funciones y cinco dominios** de otros sistemas, y una de las ocho hacía morir la migración sobre una base sin `unaccent`. Y desde #113, **`V4__particiones_de_auditoria_2028_2035.sql`**: `V1` sólo creó `auditoria_2026` y `auditoria_2027`, sin partición `DEFAULT` —a propósito, para no esconder el hueco—, y nadie creaba las siguientes; `V4` añade las ocho de 2028 a 2035, cada una con su RLS `ENABLE`+`FORCE` y su política, sin `GRANT` nuevo —el padre ya alcanza—, y `CoberturaFuturaDeParticionesDeAuditoriaTest` avisa con dos años de antelación cuando la última deje de cubrir. Y desde #111, **`V5__copia_de_identidad_por_sujeto.sql`**: `identidad_sujeto_id` en `usuario` y `grupo`, el id estable de `identidad`, nullable y único por municipalidad cuando no es nulo, para que el aplicador case por él y un renombrado no deje la fila vieja habilitada; no rellena lo existente —aquí no se sabe el id y el migrador no puede hacer `UPDATE` bajo RLS—: cada fila se adopta en su primer evento |
| `backend/kamayuk-caja-nucleo` | El contexto acotado entero: orden de cobro, ventanilla, recibo, turno, cierre, arqueo, tasas y **el buzón de salida**. Quien lo saca, `PublicadorDelBuzon`, corre desde #79 en **su propio perfil, `publicador`** —ni `web`, que tendría un publicador por réplica, ni `batch`, que termina—: un proceso de larga vida que planifica y que se queda vivo por `spring.main.keep-alive`. Y desde la etapa 4, `ClienteHttpDelBuzonDeIdentidad`: el cliente HTTP del buzón de `identidad`, que reusa `TokenDeServicioDeKeycloak` con **el mismo cliente confidencial y una segunda clave** |
...
| `infrastructure/` — el descriptor | `yarn verificar` en verde, **53 pruebas** desde #79. Declara **tres imágenes** y despliega **cuatro cosas**: el backend con el perfil `web`; desde #79 `kamayuk-caja-publicador` —el `Deployment` del perfil `publicador`, que saca el buzón de pagos: sin puerto, sin `Service` y sin sondas porque no atiende HTTP, con los `requests` de `RECURSOS_DE_ARRANQUE` y la clase `lote`, y con `KAMAYUK_CAJA_ORIGENES` compuesto con `e.namespaceDe("rentas")`, que hasta entonces era `http://rentas:8080`, un nombre de compose que el clúster no tiene—; desde la etapa 4 de ADR-0039 el `CronJob` `kamayuk-caja-consumidor-de-identidad` —`*/5 * * * *`, `Forbid`, `backoffLimit: 1`, la clase `lote`, **sin `suspend`**, con las cuatro `KAMAYUK_IDENTIDAD_*` y la segunda credencial de emisor `kamayuk-caja-<amb>-identidad`, y una arista de egreso a `identidad-sistema` en `kamayuk-identidad-<amb>`—; y, desde #17, `kamayuk-caja-interfaz` —su `Deployment` sin una sola variable de entorno, su `Service` y, desde #74, un `ConfigMap` que ya no copia el `nginx.conf` sino que monta `configuracion.js` con las señas del ambiente (`window.__KAMAYUK_CAJA__`: emisor, cliente `kamayuk-backoffice` y alcance)—. Su `IngressRoute` va **partido en dos rutas con `priority` explícita**: `/caja/api/v1` al backend y `/caja` a la interfaz, quitándole el prefijo con un `stripPrefix` — **y desde #44 esa segunda ruta sólo se declara fuera de `prod`** (`AMBIENTE_SIN_INTERFAZ`), que es la única asimetría entre ambientes de este archivo y lleva su decisión escrita al lado: la maqueta de ventanilla no se sirve en el dominio de producción, y el `Deployment` y el `Service` **siguen ahí**. **El responsable de la conciliación ya no falta**: C-7 le dio su campo a `EntornoDelDescriptor` y la línea 170 pone `KAMAYUK_CAJA_RESPONSABLE` desde `e.operacion.responsable` — este renglón decía lo contrario y era de antes de C-7. Lo que falta hoy vive en el otro repositorio, y está declarado en [`docs/00-gobierno/huecos-en-infrastructure.md`](docs/00-gobierno/huecos-en-infrastructure.md) |
| `docs/30-arquitectura/adr/` | **Tres ADR propios, y los tres son de su interfaz**: [ADR-0040](docs/30-arquitectura/adr/ADR-0040-la-ventanilla-se-conecta.md) —la ventanilla se conecta, sólo para leer, y la ruta de `prod` se retira la última—, [ADR-0042](docs/30-arquitectura/adr/ADR-0042-la-ventanilla-lee-su-propia-copia.md) —habla sólo con `/caja/api/v1` y el emisor, y los permisos de la sesión salen de **su** copia, nunca de `rentas`— y [ADR-0044](docs/30-arquitectura/adr/ADR-0044-la-ventanilla-anula-donde-esta-el-recibo.md) —anular se ofrece donde está el recibo, como acción, y ésa es su **primera escritura**: amplía la 0040 y la acota a una, y deja `AMBIENTE_SIN_INTERFAZ` donde está—. Lo que la caja *hace* lo siguen decidiendo ADR-0026 y ADR-0029. La reconstrucción de la interfaz que los cumple es [#74](https://github.com/hneyra/caja/issues/74) |
| `frontend/` — `caja-web` | **Rehecha en #74 con el stack y el método de `rentas-web`**, y la maqueta V6 —sus datos copiados del artboard, su ESLint sin `fetch` y sus siete arneses— **se retiró entera**. React 19.3 + Vite 7 + Tailwind 4, TanStack Query, i18next y Playwright; `@kamayuk/{api,formato,sesion,shell,ui}` y `@kamayuk/verificaciones` por `link:` desde **`../kamayuk-lib`, que tiene que estar clonado al lado**. **Habla con `/caja/api/v1` y con Keycloak, y con nadie más** (ADR-0042): la puerta PKCE va con el cliente público `kamayuk-backoffice`, y lo que la sesión puede abrir sale de los cinco `GET` de `kamayuk-caja-seguridad` —`/seguridad/{modulos,accesos}` y `/seguridad/sesion{,/permisos,/municipalidad}`—, que leen la copia local de ADR-0039; con `rentas` apagado la ventanilla abre. **Lee siempre y escribe en un sitio** ([ADR-0044](docs/30-arquitectura/adr/ADR-0044-la-ventanilla-anula-donde-esta-el-recibo.md), que amplía ADR-0040): `src/api/cliente.ts` exporta `leer` y `escribir`, y el único que llama a `escribir` es `datos/laAnulacion.ts` —lo barre `verificaciones/solo-lee.test.ts` sobre `src/` entero—; cobrar, cobrar tasas, cerrar el turno y explicar un pago siguen **declarados en el árbol y sin llamarse**, y la guarda lo comprueba buscando sus rutas en el código servido. **El árbol no sale de un artboard, que caja no tiene: sale de sus accesos y sus controladores** —**seis hojas** bajo Tesorería desde #100, y **un séptimo acceso servido por una acción**: `anulacion_recibo` no tenía hoja propia, porque su única operación es un `POST` y quien sólo lo tenía no veía ni un recibo—, y lo ata `verificaciones/el-arbol-cuadra-con-el-backend.test.ts`, que lee `CatalogoDelSistema.java` y los `*Controller.java` y exige que cada acceso del catálogo lo sirva **exactamente uno**, hoja o acción. Las pantallas las dibuja el intérprete de `@kamayuk/ui` (`kamayuk-lib`#27), y **las seis leen**: `datos/conectores.ts` pide `GET /cajas`, `/recibos`, `/recibos/{nro}/duplicado`, `/turnos/del-dia`, `/turnos/{turnoId}/cierre`, `/pagos/sin-entregar`, `/recaudacion/avance` y `/recaudacion/por-area`, formatea sin calcular —ningún total sale del cliente— y dice en el hueco lo que su lectura no trae. **De los tres huecos que #84 declaró no queda ninguno**: el recibo elegido se pide desde #99 —la lista deja elegir una fila y lo elegido vive en la ruta—, el arqueo del turno desde #97 —`GET /turnos/del-dia` publica el turno abierto de quien mira, y `cierre-caja` encadena su arqueo con ese `turnoId`; si no hay turno abierto los diez campos dicen cuál de las tres situaciones es, y lo que nadie contó dice «sin declarar» en vez de un cero— y **la conciliación desde #98**: `GET /conciliacion?fecha=` exige un día —a propósito, regla 6— y la hoja lo **ofrece elegir** con un selector cuyo valor vive en su dirección (`eleccion.enLaRuta`, `kamayuk-lib`#94), así que es otro `deLoElegido` y no un mecanismo nuevo; hasta que se elige uno, el bloque del cuadre dice que lo espera con los cuatro estados de una lectura —**ni un cero antes de tiempo**— y una fecha mal escrita se lee como el 422 que es, sin tapar la frase del turno; la línea del sistema de origen que no contestó dice **por qué no se sabe** en vez de un cero. Es además **el único campo de entrada de un bloque, y no es una escritura**: lo que escribe es la dirección, así que `catalogo.ts` no lo cuenta para `seEscribe` y el pie sigue sin ofrecer «Guardar». Las horas se dicen en `America/Lima`, porque cortar un instante UTC fecharía un cobro nocturno al día siguiente. **Y anular se hace donde está el recibo** (#100): una acción del bloque «El recibo elegido» abre un acto con motivo, quién autoriza, memorando y la observación obligatoria (regla 10, con los límites de `Observacion` leídos del `.java`), se confirma porque no se deshace, y llama a `POST /cobros/{nro}/anulacion` con el número que ya está en la ruta; quien no tenga `anulacion_recibo` ve la acción **impedida con su motivo**, nunca un botón mudo. Y para que quien sólo pueda anular encuentre el recibo, `GET /recibos` y `GET /recibos/{nro}/duplicado` lo admiten por `oTambien` —lo que **no** alcanza, escrito en el ADR: `oTambien` conserva el privilegio, así que con sólo `eliminacion` la lista sigue en 403, y la ventanilla lo dice en el motivo del botón—. **La cuenta de la barra la contesta el backend**, o la barra dice que esta copia no la conoce —lo que #44 quitó no vuelve, y lo vigila `la-cuenta-no-se-inventa.test.tsx`—. **Y resiste** ([#117](https://github.com/hneyra/caja/issues/117)): el reparto corre en el `select` de TanStack Query, así que un dato malformado deja **esa** hoja en fallo con su frase y no desmonta la raíz; cada pantalla va dentro de `LaHojaNoSePudoDibujar`, con `reinicio` atado a la **ruta de la hoja** —no solo al destino— para que un recibo elegido que rompe el dibujo no quede rota al elegir otro; y un 401 al anular dice que la sesión caducó y que hay que volver a entrar, con lo escrito guardado en `sessionStorage` (`datos/borradorDeLaAnulacion.ts`, **el único** archivo de `src/` que toca el almacenamiento, con una lista **exacta** de lo que importa, y nunca el token) por la **cuenta** de `GET /seguridad/sesion` —la de otra cuenta se descarta— hasta anular, cerrar el acto —salvo cerrar el rechazo de un 401, que no cuenta como cancelar— o cerrar la sesión, antes de irse al emisor (ver [ADR-0044 §Decisión·4](docs/30-arquitectura/adr/ADR-0044-la-ventanilla-anula-donde-esta-el-recibo.md)). `yarn verificar` da `Test Files 41 passed (41)` · `Tests 491 passed (491)` (#117) y `yarn e2e` **42 caminos** (#100 —entra el que anula y sale el destino que se retiró—), medidos con **Node 24** desde #93 —el `TypeError` del `AbortSignal` que ataba esto a Node 22 no era ruido: lo tapaba `handlePop`, y lo arregla el `Request` del arnés de `frontend/vitest.setup.ts`— |
...
- **No le pregunta nada a nadie para cobrar.** `CajaController` no inyecta un solo puerto hacia
  otro sistema; el consumidor del buzón de `identidad` (etapa 4 de ADR-0039) corre en el perfil
  `batch` y el publicador de pagos en el perfil `publicador` (#79), los dos fuera del camino del
  cobro: la ventanilla cobra con `rentas` apagado **y** con `identidad`
...
frontend/                       caja-web: la interfaz de ventanilla, con el stack de rentas-web (#74)
  src/                              lo que se despliega. Es la ruta que vigila la guarda del registro
    api/                              la puerta PKCE (@kamayuk/sesion) y el cliente: leer, y escribir
    datos/                            las lecturas de sesion y catalogo, la unica escritura (#100) y
                                      sus capturas (solo pruebas)
...
despliegue/                     compose.yaml: los cuatro procesos del backend —migrador,
                                implantacion, `web` y el publicador del buzon (#79)— y
                                `caja-interfaz` (#18),
...
yarn install && yarn verificar    # caja-web: ESLint (con sus muestras), tipos y Vitest. Node 24
