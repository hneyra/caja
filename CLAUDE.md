...
| **#4 — los tokens de V6** (`frontend/src/ds/`: `tokens/{colores,tipografia,formas}.css` como *custom properties*, `global.css` con el bloque `<style>` del artboard portado entero, las cuatro insignias en `ds/tokens.ts` y la carga de Source Sans 3 en `index.html`) | Dos roturas, cada una sola y restaurada por copia comparada con `cmp`: (1) `--azul: #005284` → `#005285`, la que pide el issue; (2) borrar de `global.css` la **regla** `input:focus …`, que es lo que de verdad se protege | **3 en rojo** la primera, desde tres ángulos que no se solapan: la tabla de tokens («expected '#005285' to be '#005284'»), el inventario que exige que **ningún** hexadecimal declarado en `tokens/` esté fuera de la lista —lo que caza un color *nuevo*, cosa que la tabla no puede— y el `borderColor` de un `<input>` **enfocado de verdad**, que dice que el token no sólo está declarado sino que llega al elemento. **3 en rojo** la segunda: «expected '' to be '0 0 0 3px #D3EBFA'». **Y la lección de verdad de este issue es una afirmación falsa que estuvo a punto de quedarse escrita aquí.** La primera versión cambiaba el entorno de la prueba a happy-dom sosteniendo que «el `getComputedStyle` de jsdom ignora toda regla con pseudo-clase». **Es falso, y lo destapó la revisión midiéndolo**: jsdom aplica `:focus` y devuelve `0 0 0 3px #D3EBFA`. Lo que había engañado era el **orden de la medición**: el entorno **memoriza el estilo calculado de cada elemento** la primera vez que se le pide y `focus()` no invalida esa memoria, así que la sonda original —que leyó antes de enfocar— vio `""` y se le atribuyó a la pseudo-clase. Medido después dentro de un solo documento jsdom: un campo leído antes de enfocarlo sigue dando `""` tras enfocarlo **y tras volver a enfocarlo**, mientras su hermano del mismo documento, enfocado sin lectura previa, devuelve el anillo. **La trampa era real; el diagnóstico, no** — y no es cosa de happy-dom, que se comporta igual. **La única diferencia medida entre los dos entornos** es que jsdom no sustituye `var()` en una propiedad calculada (devuelve el texto `var(--anillo-campo)`); se cubre con un `resolverVar` de cuatro líneas, se vuelve a jsdom y **happy-dom se desinstala**: un solo entorno y ninguna dependencia nueva. Con ella se fue una prueba que **no podía fallar por el motivo que decía comprobar** —afirmaba `navigator.userAgent` contiene «HappyDOM»—, sustituida por otra que sí mide las dos mitades y que se pondrá roja el día que jsdom resuelva `var()`. **El criterio 2 del issue está mal planteado y ya salía rojo sobre `main` antes de este PR**: su `grep -ri` busca «Inter» como subcadena y «interfaz» la contiene, así que casa con un comentario de #3 que no tiene nada de la paleta descartada; la palabra clave `interface` de TypeScript casaba también. Se comprueba de verdad con `\bInter\b`, que deja `frontend/src` limpio. **La contorsión que se había hecho para esquivarlo —declarar `ColoresDeInsignia` con `type`— se revirtió**: deformar el código para que pase una comprobación mal escrita es peor que la comprobación. Las pruebas pasan de **8 a 73** (65 nuevas) y `yarn build` emite por primera vez una hoja de estilos: `dist/assets/index-*.css`, **2,59 kB** (gzip 1,16), con la cadena de tres `@import` ya aplanada. |
