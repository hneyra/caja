...
| `frontend/` — `caja-web` | **El andamio, desde #3**: React 19 + Vite 6, un solo paquete y cero dependencias de red. `yarn verificar` encadena ESLint —con sus tres reglas propias y una muestra por regla—, `tsc` y Vitest; `yarn build` produce `frontend/dist/`. **Ninguna pantalla todavía**: se portan desde `TesoreriaV6.dc.html` en los issues siguientes |
...
| **#4 — los tokens de V6** (`frontend/src/ds/`: `tokens/{colores,tipografia,formas}.css` como *custom properties*, `global.css` con el bloque `<style>` del artboard portado entero, las cuatro insignias en `ds/tokens.ts` y la carga de Source Sans 3 en `index.html`) | Dos roturas, cada una sola y restaurada por copia comparada con `cmp`: (1) `--azul: #005284` → `#005285`, la que pide el issue; (2) borrar de `global.css` la **regla** `input:focus …`, que es lo que de verdad se protege | **3 en rojo** la primera, y desde tres ángulos que no se solapan: la tabla de tokens («expected '#005285' to be '#005284'»), el inventario que exige que **ningún** hexadecimal declarado en `tokens/` esté fuera de la lista —que es lo que caza un color *nuevo*, cosa que la tabla no puede— y, la que importa, el `borderColor` de un `<input>` **enfocado de verdad**: dice que el token no sólo está declarado, sino que llega al elemento. **2 en rojo** la segunda: «expected '' to be '0 0 0 3px #D3EBFA'». **Y el criterio 3 era inverificable con el andamio tal como se heredó de #3.** Medido, no supuesto: el `getComputedStyle` de **jsdom ignora toda regla que lleve una pseudo-clase**, así que un `<input>` enfocado devuelve exactamente el mismo estilo que uno sin enfocar —con `matches(':focus')` en `true`, que es lo que lo hace engañoso—, y la única prueba posible habría sido leer el texto del CSS, que es justo lo que el issue prohíbe. Se cambia el entorno **de un solo archivo** a happy-dom, con el motivo escrito dentro y afirmado por una prueba que se pondrá roja el día que jsdom aplique pseudo-clases y la excepción sobre. **Tres trampas más, las tres descubiertas ejecutando.** (1) happy-dom **memoriza** el estilo calculado de cada elemento y `focus()` no invalida esa memoria: un `getComputedStyle` antes del `focus()` devuelve `""` para siempre, de modo que la prueba escrita en el orden natural —leer, enfocar, volver a leer— habría concluido que la regla no existe teniéndola delante. (2) Sin `css: true` en Vitest, `import "…/global.css"` **no inyecta nada** y la prueba mediría un documento sin estilos: verde, y sin haber verificado nada — el mismo modo de fallo que la página vacía de P5D. (3) happy-dom expande la abreviatura `outline` a `"none none"`, que no es lo que declara el CSS ni lo que devolvería un navegador; se afirma sobre `outlineStyle`. **Y el criterio 2, tal como está escrito, no lo puede cumplir ningún archivo en español: ya estaba rojo sobre `main` antes de este PR.** Su `grep -ri` busca «Inter» como subcadena y «interfaz» la contiene, así que casa con un comentario de `aplicacion.ts` que es de #3 y no tiene nada de la paleta descartada; la palabra clave `interface` de TypeScript casaba también, y por eso `ColoresDeInsignia` se declara con `type`. Sobre **todo lo que este PR añade** (`frontend/src/ds/`) el grep literal sale limpio, y con `\bInter\b` sale limpio `frontend/src` entero. Las pruebas pasan de **8 a 73** (65 nuevas) y `yarn build` emite por primera vez una hoja de estilos: `dist/assets/index-*.css`, **2,59 kB** (gzip 1,16), con la cadena de tres `@import` ya aplanada. |
