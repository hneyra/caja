...
/**
 * **De donde salen los valores: el `@theme` de `@kamayuk/ui`** (#74).
 *
 * En `rentas` salen de `diseno/rentas-tokens.css`, la hoja del artboard V8 vendorizada en su arbol.
 * `caja` no tiene artboard, y copiar la hoja de otro sistema seria una tercera copia de la paleta.
 * Lo que se lee es el `@theme` de la libreria, que es la paleta que se sirve: la pregunta sigue
 * siendo la de #139 —que la utilidad exista con su selector exacto y declare ese valor—, y lo que
 * se pierde es comparar la libreria contra el artboard, que es trabajo de la libreria.
 */
const HOJA_DEL_ARTBOARD = HOJA_DE_UI;
...
const COLORES_DEL_ARTBOARD = [
  ...readFileSync(HOJA_DEL_ARTBOARD, 'utf8')
    .replace(/\/\*[\s\S]*?\*\//g, ' ')
    .matchAll(/--color-([a-z0-9-]+)\s*:\s*(#[0-9a-f]{6}|rgba\([^)]*\))\s*;/gi),
