 ...
 *
 * Acepta las dos formas: la de la hoja del artboard de `rentas` (`--tinta-4`) y la del `@theme` de
 * `@kamayuk/ui` (`--color-tinta-4`), que es la que lee `caja` (#74).
 */
export function valorDelToken(hoja: string, token: string): string | null {
  for (const nombre of [`color-${token}`, token]) {
    const encontrado = new RegExp(`--${nombre}\\s*:\\s*(#[0-9a-f]{6})\\s*;`, 'i').exec(hoja);
    if (encontrado?.[1] !== undefined) return encontrado[1].toLowerCase();
  }
  return null;
}
...
export function losQueNoSonTexto(hoja: string): TokenQueNoEsTexto[] {
  ...
  const anadir = (token: string, porQue: string) => {
    const nombre = token.toLowerCase();
    const valor = valorDelToken(hoja, nombre);
    if (valor === null || salida.some((t) => t.token === nombre)) return;
    salida.push({ token: nombre, valor, clase: `text-${nombre}`, porQue: porQue.replace(/\s+/g, ' ') });
  };
  // La hoja del artboard de `rentas`: un comentario por token, con la frase «NO se usa como texto».
  for (const comentario of comentariosDe(hoja)) {
    if (!/NO se usa como texto/i.test(comentario)) continue;
    for (const [, token] of comentario.matchAll(/--([a-z0-9-]+)/gi)) anadir(token ?? '', comentario);
  }
  // La hoja de `@kamayuk/ui`, que es la que lee `caja` (#74): un titulo «`--tinta-4` NO ES COLOR DE
  // TEXTO» dentro de su cabecera. Se busca el titulo y no cualquier token del comentario: esa
  // cabecera nombra otros diez tokens que si son texto.
  for (const [titulo, token] of hoja.matchAll(/`--([a-z0-9-]+)`\s+NO ES COLOR DE TEXTO[^\n]*/g)) {
    anadir(token ?? '', titulo);
  }
  return salida;
}
