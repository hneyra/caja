...
/** Los `.java` de produccion del backend: los de `src/main`, y nada de `build/`. */
function fuentesJava(desde: string): readonly string[] {
  return readdirSync(desde).flatMap((entrada) => {
    const ruta = join(desde, entrada);
    if (statSync(ruta).isDirectory()) return entrada === 'build' || entrada === 'test' ? [] : fuentesJava(ruta);
    return entrada.endsWith('.java') && ruta.includes(`${sep}src${sep}main${sep}`) ? [ruta] : [];
  });
}
...
describe('#115 — ninguna opcion del menu de sesion se queda muda', () => {
  ...
  it('y la salida elegida es honesta: el backend de la ventanilla NO publica perfil ni contrasena', () => {
    // La otra mitad de la decision, y la que impide que alguien «complete» esto con un formulario.
    // En `rentas` se mira su contrato generado; `caja` no lo tiene y no le hace falta: se leen los
    // `@*Mapping` de su propio backend, en este mismo arbol (#74). Dibujar aqui un formulario de
    // clave seria prometer una escritura que ningun backend de este repositorio puede atender. Esta
    // prueba caduca sola el dia que alguna operacion lo publique: entonces sale roja.
    const rutas = fuentesJava(join(RAIZ, '../backend')).flatMap((ruta) =>
      [...readFileSync(ruta, 'utf8').matchAll(/@(?:Get|Post|Put|Patch|Delete|Request)Mapping\(([^)]*)\)/g)].map(
        (casado) => casado[1] ?? '',
      ),
    );
    expect(rutas.length, 'no se leyo ni un mapeo del backend: la guarda no mira nada').toBeGreaterThan(15);
    expect(rutas.filter((r) => /contrasena|password|credencial|perfil/i.test(r))).toEqual([]);
  });
