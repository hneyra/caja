...
const ARBOLES = [
  ...
  '/home/jorge/ws/caja-b2/frontend',
  '/home/jorge/ws/caja-74/frontend',
  '/home/jorge/ws/caja-75/frontend',
  '/home/jorge/ws/caja-77/frontend',
  '/home/jorge/ws/caja-80/frontend',
];
...
describe('el puerto del arnes', () => {
  ...
  it('en CI es el de siempre, porque alli solo hay un arbol', () => {
    expect(elegirElPuerto({ raiz: '/home/runner/work/caja/caja/frontend', ci: true })).toEqual({
      puerto: 4173,
  ...
  it('fuera de CI NUNCA es el de CI, aunque el arbol este en la misma ruta', () => {
    expect(
      elegirElPuerto({ raiz: '/home/runner/work/caja/caja/frontend', ci: false }).puerto,
    ).not.toBe(4173);
  });
  ...
  it('el de ESTE arbol es el que lleva la URL con la que se mide', () => {
    expect(URL_DEL_ARNES).toBe(`http://localhost:${PUERTO}/caja/`);
    // `/caja/` es la `base` de `vite.config.ts`: con otra, `preview` contesta 404 en la raiz.
