...
import { PantallaDelSistema as Pantalla } from '../src/pantallas/PantallaDelSistema.tsx';
 ...
 * todo lo que traduce entre `⟦` y `⟧`, se dibujan todas las pantallas, y **lo que salga sin
 ...
 */
// La cuenta y la entidad NO son palabras de la interfaz: son datos que contesta el backend de la
// ventanilla (`useCuentaDeLaSesion.ts`), igual que el rotulo de su modulo. En `rentas` aqui habia un
// nombre escrito a mano, «J. Cardenas Vega»; en `caja` lo que se exime es lo que el doble contesta.
const NO_ES_TEXTO = new Set(['—', '/', '·', ':', '…', SESION_MEDIDA.nombre, 'AD', MUNICIPALIDAD_MEDIDA.nombre]);
...
describe('ninguna cadena llega al DOM sin pasar por `t()`', () => {
  ...
  it('y el CENTINELA de la otra direccion: con el idioma normal NO hay marcas', async () => {
    ...
    render(
      <Pantalla
        definicion={pantallaDe('cierre-caja')}
        datos={{ ausencia: porQueNoHayDato(hojaDe('cierre-caja')) }}
      />,
    );
    expect(screen.queryByText(new RegExp(ABRE))).toBeNull();
    expect(screen.getByText('Arqueo del turno')).toBeTruthy();
    await i18n.changeLanguage(IDIOMA_MARCADO);
  });
});
 ...
 *
 * Las de arriba montan `<Pantalla>` **suelta**. Eso mide el CUERPO, que es lo que este
...
const MUESTRAS: Readonly<Record<string, readonly unknown[]>> = {
  avisosSinLeer: [3],
  nadaCasaEnElArbol: ['recibo'],
  cuantosDestinos: [1, 7],
  hayCambiosSinGuardar: ['Cierre y arqueo de caja'],
};
