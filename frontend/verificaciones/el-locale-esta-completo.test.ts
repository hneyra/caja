...
const LITERALES = [
  ...
  // La costura de la ventanilla (#74): el titulo, las cuatro opciones del menu y lo que la barra
  // dice cuando todavia no sabe quien es la cuenta. La cuenta y la entidad NO estan: son datos.
  'Caja',
  'Mi perfil',
  'Cambiar la contrasena',
  'Preferencias',
  'Cerrar sesion',
  'La ventanilla de Tesorería: lo que esta cuenta puede abrir en esta caja.',
  'Identificando la cuenta…',
  'Cuenta sin identificar',
  'Cuenta que esta caja todavia no conoce',
  'Municipalidad sin identificar',
  // Las cuatro del catalogo filtrado por permisos (`rentas`#105), con las palabras de la ventanilla.
  'Averiguando que puede abrir esta cuenta.',
  'La sesion no vale para saber que puede abrir esta cuenta. Vuelva a entrar.',
  'No se pudo saber que puede abrir esta cuenta, asi que no se ofrece nada. Ofrecerlos todos ante un fallo convertiria un problema de red en un agujero de autorizacion.',
  'Esta cuenta no puede abrir nada en esta ventanilla. No es un fallo: es una cuenta sin permisos, o afiliada a un grupo que no los tiene.',
  // Las tres de la puerta que no contesta (`rentas`#112).
...
describe('el locale `es` esta completo y no se aparta', () => {
  ...
  it('EL CENTINELA: el catalogo derivado trae mas de un centenar de claves', () => {
    ...
    // Siete pantallas y el marco, no las cuarenta de `rentas`: medido al regenerar (#74), ciento y pico.
    expect(Object.keys(esperado).length, 'el catalogo vino corto').toBeGreaterThan(100);
  });
