 ...
 *
 * La plataforma no esta levantada —ni puede estarlo en CI—, asi que `localhost:8180` no contesta.
...
test('«Cambiar la contrasena» abre la pagina de credenciales, no una pantalla de la ventanilla', async ({
  page,
}) => {
  await abrir(page, 'cierre-caja');
...
test('y las dos URL cuelgan del MISMO origen por el que se entra: no hay una segunda seña', async ({
  ...
  // habria entrado a la ventanilla: el formulario de identificacion se sirve del mismo sitio.
  await abrir(page, 'cierre-caja');
