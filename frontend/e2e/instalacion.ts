 ...
 *
 * Si una pantalla pidiera datos, tendria que verse **en su estado de error**,
 ...
 */
const IDAS = 'kamayuk.caja.pkce.idas';
...
export async function conLaSeguridadContestada(pagina: Page): Promise<void> {
  ...
  await pagina.route('**/caja/api/v1/**', async (ruta) => {
...
export async function abrir(pagina: Page, clave: string): Promise<void> {
  ...
  // La interfaz no monta el armazon hasta saber que puede abrir la cuenta: antes no hay barra.
