...
describe('el borrador de la anulacion', () => {
  ...
  it('un valor ilegible o con otra forma no es un borrador', () => {
    ...
    expect(leerLosBorradores(CUENTA)).toEqual({});
    sessionStorage.setItem(
      CLAVE_DEL_BORRADOR,
      JSON.stringify({ cuenta: CUENTA, actos: { [APERTURA]: { valores: { motivo: 3 }, observacion: 'x', intentado: false } } }),
    );
    expect(leerLosBorradores(CUENTA)).toEqual({});
  });

  it('se guarda CON la cuenta que lo escribio, y nunca con nada mas de la sesion', () => {
    guardarLosBorradores(CUENTA, { [APERTURA]: ESCRITO });
    expect(JSON.parse(sessionStorage.getItem(CLAVE_DEL_BORRADOR) ?? '{}')).toEqual({
      cuenta: CUENTA,
      actos: { [APERTURA]: ESCRITO },
    });
  });

  it('el de otra cuenta no se devuelve, y se borra de la pestana', () => {
    guardarLosBorradores('otro.cajero', { [APERTURA]: ESCRITO });
    expect(leerLosBorradores(CUENTA)).toEqual({});
    expect(sessionStorage.getItem(CLAVE_DEL_BORRADOR)).toBeNull();
  });

  it('olvidarlos quita la clave entera', () => {
    guardarLosBorradores(CUENTA, { [APERTURA]: ESCRITO });
    olvidarLosBorradores();
    expect(sessionStorage.getItem(CLAVE_DEL_BORRADOR)).toBeNull();
  });
