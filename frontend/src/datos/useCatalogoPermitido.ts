...
/**
 * **El catalogo que el armazon recibe, filtrado por lo que la cuenta puede abrir** (`rentas`#105;
 * en `caja` desde #74, contra su propio backend y hoja por hoja: ver `permisos.ts`).
 *
...
export const LLAVES = {
  ...
  // Las dos de la cuenta, que no filtran nada pero viven en la misma rama: la siembra las pone
  // igual, y un solo sitio para las llaves es lo que impide que se desincronicen. Ver
  // `useCuentaDeLaSesion.ts`.
  sesion: [RAMA, 'sesion'],
...
export function useCatalogoPermitido(): CatalogoDeLaSesion {
    ...
    return {
      ...
      porQue:
        codigo === 401
          ...
          : t(
              'No se pudo saber que puede abrir esta cuenta, asi que no se ofrece nada. ' +
  ...
  if (compuesto.catalogo.length === 0) {
    return {
      ...
      estado: 'sin-permiso',
      porQue: t(
        'Esta cuenta no puede abrir nada en esta ventanilla. No es un fallo: es una cuenta ' +
