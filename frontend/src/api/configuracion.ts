 ...
 * de quien desarrolla (`localhost:8180`), en la marcha blanca y en la municipalidad, y una URL
 ...
 * todo arnes montara un `window.__KAMAYUK_CAJA__` para dibujar una pantalla que no entra a
...
declare global {
  interface Window {
     ...
     */
    __KAMAYUK_CAJA__?: Partial<Record<ClaveDeConfiguracion, string>>;
  }
}
 ...
 *
 * `kamayuk-backoffice` y `localhost:8180` son los que hacen que `yarn dev:con-plataforma` entre por
 * la puerta sin configurar nada. **El 8180 no es el 8181 de `rentas`**, de donde se copio este
 * archivo (#74): es el `${KAMAYUK_PUERTO_IDENTIDAD:-8180}` de
 * `infrastructure/despliegue/plataforma.compose.yaml` y el `KAMAYUK_OIDC_EMISOR` por omision de
 * `despliegue/compose.yaml`, y que los tres digan lo mismo lo comprueba
 * `verificaciones/camino-a-la-api.test.ts`.
 */
const POR_OMISION: Record<ClaveDeConfiguracion, string> = {
  oidcRealm: 'http://localhost:8180/realms/kamayuk',
...
function servida(clave: ClaveDeConfiguracion): string | undefined {
  ...
  return siTieneAlgo(window.__KAMAYUK_CAJA__?.[clave]);
}
