 ...
 *
 * En `rentas`, `verificaciones/pantallas-del-artboard.test.ts` compara sus 40 definiciones **campo por
 * campo y literal** contra su artboard, y en `caja` `el-arbol-cuadra-con-el-backend` compara los
 * rotulos con el catalogo del backend. Es la guarda que hace cierto «debe lucir identico».
 *
 * Con claves opacas —`'cierre-caja.b0.c1'`— esa comparacion muere: compararia claves contra
 * castellano. Y ensenandola a resolver, sus rojos pasarian de decir
 *
 *     bloque 0 · campo 1: «Puede cerrarse» tipo «r»
 *
 * a decir «cierre-caja.b0.c1», que es mucho peor de leer justo cuando mas falta hace.
 *
 * Asi que la clave **es** el castellano: la etiqueta del campo, tal cual. La guarda queda intacta, la
 * definicion sigue siendo la fuente de verdad, y **un segundo idioma es un JSON** que mapea
